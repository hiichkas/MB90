...
&nbsp;<a href="<?php echo admin_url("post.php?post={$selected_page_id}&action=edit"); ?>" target="_blank" class="button"><?php _e('Edit', 'memberpress'); ?></a>
  ...
  public static function payment_currencies_dropdown($field_name, $payment_currency) {
    $payment_currencies = MeprHooks::apply_filters('mepr-currency-symbols', array( '$', '£', '€', '¥', 'kr', 'R$', '฿', '₹', 'zł', 'лв', 'Ft', 'Rp', 'R', '₪', '﷼', 'CHF', 'KSh', 'RM', 'руб', 'NT$', 'Mex$', 'P', 'lei', 'JOD', ''));
  ...
  public static function payment_currency_code_dropdown($field_name, $code) {
    $codes = MeprHooks::apply_filters('mepr-currency-codes', array( 'USD', 'GBP', 'EUR', 'JPY', 'AUD', 'BRL', 'CAD', 'HKD', 'NZD', 'SGD', 'SEK', 'THB', 'INR', 'PLN', 'BGN', 'HUN', 'HUF', 'IDR', 'ZAR', 'NOK', 'DKK', 'ILS', 'SAR', 'CHF', 'KES', 'MYR', 'RUB', 'TWN', 'MXN', 'BWP', 'RON', 'JOD'));
    $field_value = isset($_POST[$field_name])?$_POST[$field_name]:null;
  ...
  public static function payment_language_code_dropdown($field_name, $code) {
    $codes = MeprHooks::apply_filters('mepr-language-codes', array( 'US', 'AU', 'CN', 'EN', 'FR', 'BR', 'DE', 'IT', 'JP', 'ES', 'GB', 'SE', 'TH', 'PL', 'BG', 'HU', 'ID', 'PT', 'ZA', 'NO', 'DK', 'HE', 'FI', 'SR', 'CH', 'SW', 'NL', 'MS', 'RU', 'TW', 'MX', 'TN', 'RO', 'AR', 'SK' ));
    $field_value = isset($_POST[$field_name])?$_POST[$field_name]:null;
