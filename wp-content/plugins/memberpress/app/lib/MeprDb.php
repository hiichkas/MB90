...
class MeprDb
{
  ...
  public static function list_table( $cols,
    ...
    // Setup Joins
    $inner_joins = $outer_joins = array();

    if(!empty($joins)) {
      foreach($joins as $join) {
        if(preg_match('/^\s*(LEFT|OUTER)/', $join)) {
          $outer_joins[] = $join;
        }
        else {
          $inner_joins[] = $join;
      ...
      $outer_join_str = " ".implode(" ", $outer_joins);
      $inner_join_str = " ".implode(" ", $inner_joins);
    ...
    $total_query = "SELECT COUNT(*) FROM {$from}{$inner_join_str}{$conditions}";
