...
/*
    ...
    function pollVisibility() {
        if ($(".outer-timer-wrapper").css("display") == "block") {
          $( ".exerciseListItem" ).each(function(){
              //exScrollerWidthArray.push($(this).css("width"));
              exScrollerWidthArray.push($(this).width() + 8); // add 4px for the padding
              totScrollDistance += $(this).width() + 8;
          });  
        } else {
    ...
    $('.horizon-next').on('click', function(event) {
        ...
        $('#ex-scroller-content').parent().css({position: 'relative'});
        ...
        $(".exerciseListItem > button").eq(exScrollerIndex).addClass("current-exercise animated"); // reset the previous button colour

    });

    // setup an event to rewind the exercise scroller when use clicks stop
    $('body').on('click', 'i.stop-button', function() {
        ...
        $(".exerciseListItem > button").eq(exScrollerIndex).addClass("current-exercise animated"); // add the current exercise class to the 1st exercise
        $(".exerciseListItem > button").eq(exScrollerIndex).removeClass("unselected-exercise"); // remove the unselected class
