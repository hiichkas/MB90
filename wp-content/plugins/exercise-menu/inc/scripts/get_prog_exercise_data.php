...
//$progExercises = $wpdb->get_results("SELECT * FROM mb90_prog_exercises_translated WHERE ProgrammeID = " .$progSelected. " ORDER BY ProgrammeID, ExerciseDay OrderNumber limit $offset,$pageRows", OBJECT);
$progExercises = $wpdb->get_results("SELECT * FROM mb90_prog_exercises_translated WHERE ProgrammeID = " .$progSelected. " ORDER BY ProgrammeID, ExerciseDay, OrderNumber, ExerciseName", OBJECT);
...
if( $wpdb->num_rows > 0 )
{
    ...
    {
        if( $row->ExerciseDay == 1){
            $row->ExerciseDay = "SA";
        }
        elseif( $row->ExerciseDay == 8){
            $row->ExerciseDay = "10 Day";
        }
        $rows[] = $row;
    }
